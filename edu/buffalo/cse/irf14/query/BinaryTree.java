	...
	public List<Integer> categoryLookUp(String term, boolean not){
		try{
			...
			if(dir.exists()){
				...
				Map<Integer, TermData> index = (TreeMap<Integer, TermData>) ois.readObject();
				ois.close();
				...
				if(not){
					...
					for(Entry<Integer, String> entry : this.documentDictionary.entrySet()){
					...
					if(id != null){
						...
						if(termData != null)
							return index.get(dictionary.get(term)).getPostingsList();
					}
				}
			}
		}
	...
	public List<Integer> authorLookUp(String term, boolean not){
		...
		try{
			...
			if(dir.exists()){
				...
				Map<String, String> dictionary = (TreeMap<String, String>) ois.readObject();
				...
				Map<Integer, List<Integer>> index = (TreeMap<Integer, List<Integer>>) ois.readObject();
				ois.close();
